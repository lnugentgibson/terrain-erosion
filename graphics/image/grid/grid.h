...
template <typename T, typename C> class TypedGrid {
  T *data;

public:
  const int rows, cols, dim;
  TypedGrid(int _rows, int _cols, int _dim)
      : rows(_rows), cols(_cols), dim(_dim), data(0) {}
  ~TypedGrid();
  void allocate();
  ...
  // friend TypedGrid combine<T, T1, T2, C>(const char *filename1, const char
  // *filename2, int dim, void (*combiner)(const T1 *a, const T2 *b, T *c));
};
...
bool TypedGrid<T, C>::readTxt(const char *filename) {
  ...
  ifs >> _cols;
  if (rows != _rows) {
    std::cout << "Invalid row count: " << rows << "(expected) " << _rows
              << "(actual)" << std::endl;
    return false;
  }
  if (cols != _cols) {
    std::cout << "Invalid col count: " << cols << "(expected) " << _cols
              << "(actual)" << std::endl;
    return false;
  }
  allocate();
...
template <typename T, typename C> TypedGrid<T, C>::~TypedGrid() {
  if (data != 0) {
    delete[] data;
  }
}

template <typename T, typename C> void TypedGrid<T, C>::allocate() {
  if (data != 0) {
    return;
  }
...
template <typename T, typename C> T &TypedGrid<T, C>::getFirst(int i, int j) {
...
template <typename T, typename C> T *TypedGrid<T, C>::get(int i, int j) {
  return &data[(i * cols + j) * dim];
}
...
const T *TypedGrid<T, C>::get(int i, int j) const {
  ...
  // std::cout << "get(" << i << ", " << j << ") -> &data[" << index << "]" <<
  // std::endl;
  return &data[index];
}

template <typename T, typename C> T &TypedGrid<T, C>::get(int i, int j, int k) {
...
template <typename T, typename C> T *TypedGrid<T, C>::set(int i, int j, T *v) {
  for (int k = 0; k < dim; k++) {
