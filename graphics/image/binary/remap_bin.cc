...
int main(int argc, char *argv[]) {
  ...
  options.add_options()("i,input", "first binary image file path",
                        cxxopts::value<std::string>())("f,from-min", "from min",
                                                       cxxopts::value<float>())(
      "g,from-max", "from max", cxxopts::value<float>())(
      "t,to-min", "to min", cxxopts::value<float>())("u,to-max", "to max",
                                                     cxxopts::value<float>())(
  ...
  Map(InputSpecifier(&ifs, sizeof(float)), OutputSpecifier(&ofs, sizeof(float)),
      transformer.get());
  ifs.close();
  ofs.close();
  return 0;
