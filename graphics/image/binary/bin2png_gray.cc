...
public:
  ...
  void Do(int i, int j, int rows, int cols,
          const PixelSpecifier pixel) override {
...
int main(int argc, char *argv[]) {
  ...
  auto builder = ColorizerFactory::get().Create("GrayscaleColorizer");
  auto colorizer = (*builder)();
  uint8_t *pixels = new uint8_t[cols * rows * 3];
  ForEach(InputSpecifier(&ifs, sizeof(float)),
          new ToPNG(colorizer.get(), pixels));
  ifs.close();
  stbi_write_png(result["o"].as<std::string>().c_str(), cols, rows, 3, pixels,
                 cols * 3);
  delete[] pixels;
  return 0;
}