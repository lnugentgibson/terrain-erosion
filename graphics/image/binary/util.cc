...
public:
  ScalingTransformer(float fn, float fx, float tn, float tx)
      : from_min(fn), from_max(fx), from_range(from_max - from_min), to_min(tn),
        to_max(tx), to_range(to_max - to_min) {}
...
class MinMaxAccumulator : public image::binary::Accumulator {
  void Aggregate(int i, int j, int rows, int cols, const PixelSpecifier pixel,
                 int n, void *aggregate) override;
};
...
public:
  ...
  void Combine(int i, int j, int rows, int cols, const PixelSpecifier in_pixel1,
...
void ScalingTransformer::Transform(int i, int j, int rows, int cols,
                                   const PixelSpecifier in_pixel,
                                   PixelSpecifier out_pixel) {
  *reinterpret_cast<float *>(out_pixel.pixel) =
      (reinterpret_cast<const float *>(in_pixel.pixel)[0] - from_min) /
          from_range * to_range +
      to_min;
}

bool ScalingTransformerBuilder::SetFloatParam(const std::string &param,
                                              float value) {
  if (param == "fn") {
    fn = value;
    return true;
...
TransformerFactoryRegistration<ScalingTransformerBuilder>
    _ScalingTransformerBuilder("ScalingTransformer");
}

void MinMaxAccumulator::Aggregate(int i, int j, int rows, int cols,
...
AccumulatorFactoryRegistration<MinMaxAccumulatorBuilder>
    _MinMaxAccumulatorBuilder("MinMaxAccumulator");
}

void SumCombiner::Combine(int i, int j, int rows, int cols,
                          const PixelSpecifier in_pixel1,
                          const PixelSpecifier in_pixel2,
                          PixelSpecifier out_pixel) {
  *reinterpret_cast<float *>(out_pixel.pixel) =
      weight_a * reinterpret_cast<const float *>(in_pixel1.pixel)[0] +
      weight_b * reinterpret_cast<const float *>(in_pixel2.pixel)[0];
}
...
CombinerFactoryRegistration<SumCombinerBuilder>
    _SumCombinerBuilder("SumCombiner");
}
...
ColorizerFactoryRegistration<GrayscaleColorizerBuilder>
    _GrayscaleColorizerBuilder("GrayscaleColorizer");
ColorizerFactoryRegistration<SignedColorizerBuilder>
    _SignedColorizerBuilder("SignedColorizer");
ColorizerFactoryRegistration<ColorColorizerBuilder>
    _ColorColorizerBuilder("ColorColorizer");
ColorizerFactoryRegistration<VectorColorizerBuilder>
    _VectorColorizerBuilder("VectorColorizer");
ColorizerFactoryRegistration<DirectionColorizerBuilder>
    _DirectionColorizerBuilder("DirectionColorizer");
} // namespace ColorizerRegistrations
