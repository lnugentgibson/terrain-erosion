...
void ShapeTransformer::Transform(
    int i, int j, int rows, int cols,
    const PixelSpecifier in_pixel,
  ...
  p[0] = static_cast<float>(j) / cols;
  p[1] = static_cast<float>(i) / rows;
  *reinterpret_cast<float *>(out_pixel.pixel) = shape->Inside(p) ?
    1.0 :
    *reinterpret_cast<float *>(in_pixel.pixel);
}
...
bool ShapeTransformerBuilder::SetPtrParam(const std::string& param, void *value) {
  if(param == "shape") {
    shape = static_cast<Shape*>(value);
    return true;
  }
...
namespace TransformerRegistrations {
	TransformerFactoryRegistration<ShapeTransformerBuilder> _ShapeTransformerBuilder("ShapeTransformer");
}
} // namespace binary
} // namespace image
} // namespace graphics